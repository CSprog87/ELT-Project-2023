...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    order_purchase_amount_per_date = filtered_dates.groupby(filtered_dates["order_purchase_timestamp"].dt.date)["order_id"].count().reset_index()
    order_purchase_amount_per_date.columns = ["date", "order_count"]   
    
    holidays['date'] = pd.to_datetime(holidays['date'])
    order_purchase_amount_per_date['date'] = pd.to_datetime(order_purchase_amount_per_date['date'])
    
    order_purchase_amount_per_date['holiday'] = order_purchase_amount_per_date['date'].isin(holidays['date'])
    result_df = order_purchase_amount_per_date
